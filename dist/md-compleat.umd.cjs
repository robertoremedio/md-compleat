...
You will receive the full Markdown document. Return the complete document with all <ai> tags replaced by the generated content. Do not alter any other part of the document. Preserve all formatting, headings, lists, code blocks, and other Markdown syntax exactly as they appear.`}var R=S(((e,t)=>{t.exports={}})),z=class{constructor(e){if(!e.cliCommand)throw Error(`CLI provider requires a cliCommand`);this.command=e.cliCommand}async execute(e,t){if(t?.aborted)throw Error(`Aborted`);let n;try{n=(await Promise.resolve().then(()=>w(R(),1))).spawn}catch{throw Error(`CLI provider requires Node.js/Electron environment (child_process not available)`)}let r=(0,I.parse)(this.command);if(r.some(e=>typeof e!=`string`))throw Error(`CLI command contains shell operators (&&, |, ;, etc.) which are not supported. Use a wrapper script instead.`);let[i,...a]=r,o=L()+`
